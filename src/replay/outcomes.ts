import type { BusinessOutcome, Capability, OutputSpec } from "../schema/index.js";
...
async function readOutputSource(spec: OutputSpec, surface: Surface): Promise<string | null> {
  ...
  if (source.kind === "text_of") {
    ...
    return surface.readText(source.target);
  }
  if (source.kind === "attribute_of") {
    ...
    return surface.readAttribute(source.target, source.attribute);
  }
